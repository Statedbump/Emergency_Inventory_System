...
create table payment(payment_id serial primary key,payment_type varchar(100),payment_total float,
		     o_id integer references order(o_id));
create table order(o_id serial primary key,o_date DATE,o_quantity integer,r_list TEXT);
...
create table buys(r_id integer references resource(r_id), o_id integer references order(o_id),total_price float,
